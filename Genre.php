...
$query ="Select Code_Genre,Libellé_Abrégé FROM Genre";
echo "<h1 align=center> Tous les genres</h1>";
echo "</br></br>";
echo "<table align=center>";
foreach ($pdo->query($query) as $row) {
  $query1="Select Code_Album FROM Album inner join Genre ON Genre.Code_Genre=Album.Code_Genre WHERE Album.Code_Genre=".$row['Code_Genre'];
  $pdo1 = new PDO($pdodsn, $user, $password);
  if ($pdo1->query($query1)->fetchColumn() != "")
    echo "<tr><td><a href=Album.php?Genre=".$row['Code_Genre']."&nom=".urlencode($row[utf8_decode('Libellé_Abrégé')]).">".$row[utf8_decode('Libellé_Abrégé')]."</td></tr>"; 
  else{
    $pdo1=null;
    continue;
  }
