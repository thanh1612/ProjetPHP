        ...
        <li class="dropdown">
          ...
          <ul class="dropdown-menu"> -->
            ...
            if (empty($_SESSION["NOM_USER"]))
              header("Location: Connexion.php?url=Panier.php");
            else
...
<br><br><br><br><br>
<?php
if (isset($_COOKIE["Code_Morceau"]) && $_COOKIE["Code_Morceau"]!="")
{
  ...
  $query = "Select Credit From Abonné Where Code_Abonné=".$_SESSION['CODE_USER'];
  foreach ($pdo->query($query) as $row)
      if ($row['Credit']=="")
        $_SESSION['Credit']=0;
      else $_SESSION['Credit']=$row['Credit'];
  echo "<center><h3>VOTRE PANIER</h3></center></br></br>";
  echo "<table border=1 align=center> <tr align=center> <td> <h3> Titre du morceau</h3> </td>
  <td> <h3>Extrait</h3> </td><td> <h3>Prix Unitaire</h3> </td><td colspan=2> Vos Crédits: ".$_SESSION['Credit']."</td></tr>";
  $ID = explode(",", $_COOKIE["Code_Morceau"]);
  $prix=0;
  for ($i=0; $i < count($ID); $i++) 
  { 
    $query="Select Titre, Prix FROM Enregistrement Where Code_Morceau=".$ID[$i];
      foreach ($pdo->query($query) as $row) 
      { 
        echo "<tr align=center><td>". $row[utf8_decode('Titre')]."</td>
        <td><audio src='Extrait.php?Code_Morceau=".$ID[$i]."' controls></audio></td>
        <td>".number_format($row['Prix'])." Crédit</td>
        <td><a href=Acheter.php?acheter=".$i.">Buy</a></td>
        <td><a href=Delete.php?delete=".$i.">Delete</a></td></tr>";
        $prix+=$row['Prix'];
      }
  }
  echo "<tr><td><h3>Prix Total: <h3></td><td></td><td><h3>".$prix." Crédit(s)</h3></td>
      <td><a href=Acheter.php>Buy All</a></td><td><a href=Delete.php>Delete All</a></td></tr>";
  $pdo=null;
}
else echo "<center><h3>Votre panier est vide !</h3></center>";
