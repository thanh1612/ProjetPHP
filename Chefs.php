...
if(isset($_REQUEST['initial']))
{
  ...
  echo "<h1 align=center> Chef d'orchestre contenant \"".$initial."\" </h1>";
  $query = "Select DISTINCT Musicien.Nom_Musicien, Musicien.Prénom_Musicien, Musicien.Code_Musicien
  FROM Musicien INNER JOIN Direction ON Musicien.Code_Musicien = Direction.Code_Musicien
...
else
{
  ...
  FROM Musicien INNER JOIN Direction ON Musicien.Code_Musicien = Direction.Code_Musicien
  Order by Nom_Musicien";
  echo "<h1 align=center> Tous les chef d'orchestre </h1>";
}
echo "</br></br>";
?>
...
if ($pdo->query($query)->fetchColumn() !="")
{
  ...
  foreach ($pdo->query($query) as $row) {
    ...
    if ($i>=$nb-10)
    {
      ...
      "</h4></td><td><h4> <a href=Album.php?Code=".$row['Code_Musicien']."&Type=Chef>Liste des albums </a></h4></td>
      <td><img src='ImageMusicien.php?Code_Musicien=".$row['Code_Musicien']."' alt='photo musicien' height=200></td></tr>";
    }
    $i+=1;
  }
}
...
/*--------------------------------------------------PAGINATION -------------------------------------------------*/
  if ($d<=50)                                                                                                               
  {
    if ($d>10)                                                      
    {
      ...
      {
        if (isset($_REQUEST['initial']))
          echo "<li><a href=Chefs.php?initial=".urlencode($initial)."&page=".($page-1)."><</a></li>";
        else 
          echo "<li><a href=Chefs.php?page=".($page-1)."><</a></li>";                                     
      }                                                            
      for ($i=1; $i*10<$d+10 ; $i++) 
      { 
        if ($i==$page)                                                                                   
          ...
          if (isset($_REQUEST['initial']))
            echo "<li><a href=Chefs.php?initial=".urlencode($initial)."&page=".$i.">".$i."</a></li>";                                   
          else echo "<li><a href=Chefs.php?page=".$i.">".$i."</a></li>";
        }
      }
      if ($page*10<$d)
        if (isset($_REQUEST['initial']))
    ...
    {
      if (isset($_REQUEST['initial'])){
        echo "<li><a href=Chefs.php?initial=".urlencode($initial)."&page=".($page-5)."><<</a></li>";                                   
        echo "<li><a href=Chefs.php?initial=".urlencode($initial)."&page=".($page-1)."><</a></li>";
        }
      else{                                                                                                      
        echo "<li><a href=Chefs.php?page=".($page-5)."><<</a></li>";                                   
        echo "<li><a href=Chefs.php?page=".($page-1)."><</a></li>";                                    
      }    
    }
    else if ($page>1)
